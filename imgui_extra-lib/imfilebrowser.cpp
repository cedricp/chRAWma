...
namespace imgui_addons
{
    ...
    bool ImGuiFileBrowser::showFileDialog(const std::string& label, const DialogMode mode, const ImVec2& sz_xy, const std::string& valid_types)
    {
        ...
        if ( ImGui::Begin(label.c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse) )
    ...
    bool ImGuiFileBrowser::showFileDialogPopup(const std::string& label, const DialogMode mode, const ImVec2& sz_xy, const std::string& valid_types)
	{
    	dialog_mode = mode;
		ImGuiIO& io = ImGui::GetIO();
		max_size.x = io.DisplaySize.x;
