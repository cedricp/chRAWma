...
struct app_impl
{
	...
	std::vector<SocketNotifier*> sockets_notifiers;
...
void  Window_SDL::set_monitor_lut(std::string lut_file)
{
	if (_impl->_monitor_lut_glid){
		glDeleteTextures(1, &_impl->_monitor_lut_glid);
	}
	...
	glGenTextures(1, &_impl->_monitor_lut_glid);
	glBindTexture(GL_TEXTURE_3D, _impl->_monitor_lut_glid);
...
bool App_SDL::handle_fd_events()
{
	...
	for(int i = 0; i < _impl->sockets_notifiers.size(); ++i){
		std::vector<int> fds = _impl->sockets_notifiers[i]->get_fds();
		for (int j = 0; j < fds.size(); ++j){
			fd_set set;
			FD_ZERO (&set);
			FD_SET (fds[i], &set);
			int retval = select(fds[i]+1, &set, NULL, NULL, &tv);
			if (retval > 0){
				_impl->sockets_notifiers[i]->data_available(fds[i]);
				event = true;
			}
		}
	}
...
void App_SDL::register_fd_event(SocketNotifier* sn)
{
	if (std::find(_impl->sockets_notifiers.begin(), _impl->sockets_notifiers.end(), sn) == _impl->sockets_notifiers.end())
		_impl->sockets_notifiers.push_back(sn);
}

void App_SDL::unregister_fd_event(SocketNotifier* sn)
{
	std::vector<SocketNotifier*>::iterator it = std::find(_impl->sockets_notifiers.begin(), _impl->sockets_notifiers.end(), sn);
	if (it != _impl->sockets_notifiers.end())
		_impl->sockets_notifiers.erase(it);
}
