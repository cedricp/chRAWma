...
void Video_base::get_frame_as_gl_texture(uint32_t frame, Texture2D& texture)
{
    if (_raw_buffers.empty()){
        _raw_buffers.resize(get_num_frames(), NULL);
    }

    uint16_t* framebuffer = NULL;
    if (_raw_buffers[frame] == NULL){
        framebuffer = get_raw_buffer(frame, _idt, _rawinfo);
    ...
    if (get_sampling_factor_y() != get_sampling_factor_x()){
...
void Video_base::set_lens_params(const std::string camera, const std::string lens, float crop_factor, float aperture, float focus_distance, float focus_length, bool do_expo, bool do_distort)
{
    ...
    _lens_op = new Lens_correction(camera, lens, resolution_x(), resolution_y(), crop_factor, aperture, focus_distance, focus_length, do_expo, do_distort);
    if(!_lens_op->valid()){
        delete _lens_op;
        _lens_op = NULL;
    }
...
{
    if (get_sampling_factor_x() == get_sampling_factor_y()){
        return raw_resolution_x();
    }
    float raw_ratio = (float)get_sampling_factor_x() / (float)get_sampling_factor_y();
